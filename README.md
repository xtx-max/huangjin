...
| 波动分析 | 收盘价 + MA20/60/120 均线叠加图、回撤子图；区间总涨幅、年化收益率、年波动率、最大回撤（全部前端计算）；区间事件归因表解释波动原因 |
| 金价预测 | 纯前端统计模型（线性回归 + Holt 双指数平滑）给出未来 30/60/90 个交易日的点预测与 90% 简化区间，含拟合优度 R²；明确标注不构成投资建议 |
| 事件时间线 | 1971–2025 年影响黄金的 37 件全球大事件，按年份分组、类别/影响筛选，每件含背景/传导机制/金价表现/启示四段式中文分析 |
| 黄金新闻 | 人工整理快讯 + 「刷新最新快讯」按钮实时抓取（东方财富公开接口，免密钥）+ GitHub Actions 每日自动抓取入档（东方财富 + Google News） |
